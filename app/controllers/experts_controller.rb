class ExpertsController < ApplicationController
  ...
  before_action :set_service_points, only: [:new, :create, :edit, :update]

  # GET /experts
  # GET /experts.json
  def index
    @q = Expert.ransack(params[:q])
    @experts = @q.result.includes(:service_point).order(:name)
  end
  ...
  def create
    ...
    respond_to do |format|
      if @expert.save
        format.html { redirect_to experts_url, notice: 'Expert was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @expert.update(expert_params)
        format.html { redirect_to experts_url, notice: 'Expert was successfully updated.' }
  ...
  private
    ...
    def set_services
      @services = Service.all_without_service_point
    end
    ...
    def expert_params
      params.require(:expert).permit(:name, :service_point_id, service_ids: [])
    end
end
